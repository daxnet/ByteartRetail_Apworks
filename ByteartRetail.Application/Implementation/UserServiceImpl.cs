    ...
    public class UserServiceImpl : ApplicationService, IUserService
    {
        ...
        public bool DisableUser(UserDataObject userDataObject)
        {
            ...
            if (!IsEmptyGuidString(userDataObject.ID))
                user = _userRepository.GetByKey(new Guid(userDataObject.ID));
            else if (!string.IsNullOrEmpty(userDataObject.UserName))
                user = _userRepository.GetUserByName(userDataObject.UserName);
            else if (!string.IsNullOrEmpty(userDataObject.Email))
                user = _userRepository.GetUserByEmail(userDataObject.Email);
            else
        ...
        {
            return PerformUpdateObjects<UserDataObjectList, UserDataObject, User>(userDataObjects, _userRepository,
                udo => udo.ID,
                (u, udo) =>
                {
        ...
        public void DeleteUsers(UserDataObjectList userDataObjects)
        {
            ...
            foreach (var userDataObject in userDataObjects)
            {
                ...
                if (!IsEmptyGuidString(userDataObject.ID))
                    ...
                    throw new ArgumentNullException("userDataObjects", "Either ID, UserName or Email should be specified.");
                var userRole = _userRoleRepository.Find(Specification<UserRole>.Eval(ur => ur.UserID == user.ID));
                if (userRole != null)
        ...
        {
            return Mapper.Map<Role, RoleDataObject>(_roleRepository.GetByKey(id));
        }
        ...
        {
            return PerformCreateObjects<RoleDataObjectList, RoleDataObject, Role>(roleDataObjects, _roleRepository);
        }
        ...
        {
            PerformDeleteObjects(roleIDs,
                _roleRepository,
                id =>
                {
                    var userRole = _userRoleRepository.Find(Specification<UserRole>.Eval(ur => ur.RoleID == id));
                    if (userRole != null)
                        _userRoleRepository.Remove(userRole);
                });
        }
